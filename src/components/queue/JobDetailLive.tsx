...
export function JobDetailLive({ initialJob }: JobDetailLiveProps) {
  ...
  const [pendingAction, setPendingAction] = useState<"cancel" | "forceUnlock" | null>(null);
          ...
          <div className="rounded-lg border p-4">
            ...
            <dl className="space-y-1 text-sm">
              ...
              <div className="flex justify-between gap-4">
                ...
                <dd className="font-mono text-xs">{typeof job.runAt === 'string' ? job.runAt : job.runAt.toISOString()}</dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-muted-foreground">Created</dt>
                <dd className="font-mono text-xs">{typeof job.createdAt === 'string' ? job.createdAt : job.createdAt.toISOString()}</dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-muted-foreground">Updated</dt>
                <dd className="font-mono text-xs">{typeof job.updatedAt === 'string' ? job.updatedAt : job.updatedAt.toISOString()}</dd>
              </div>
            </dl>
          </div>
          ...
          <div className="rounded-lg border p-4">
            ...
            <dl className="space-y-1 text-sm">
              ...
              <div className="flex justify-between gap-4">
                ...
                <dd className="font-mono text-xs">{job.lockedAt ? (typeof job.lockedAt === 'string' ? job.lockedAt : job.lockedAt.toISOString()) : "—"}</dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-muted-foreground">Lease Expires</dt>
                <dd className="font-mono text-xs">{job.lockExpiresAt ? (typeof job.lockExpiresAt === 'string' ? job.lockExpiresAt : job.lockExpiresAt.toISOString()) : "—"}</dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-muted-foreground">Cancel Requested</dt>
                <dd className="font-mono text-xs">{job.cancelRequestedAt ? (typeof job.cancelRequestedAt === 'string' ? job.cancelRequestedAt : job.cancelRequestedAt.toISOString()) : "—"}</dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-muted-foreground">Cancelled At</dt>
                <dd className="font-mono text-xs">{job.cancelledAt ? (typeof job.cancelledAt === 'string' ? job.cancelledAt : job.cancelledAt.toISOString()) : "—"}</dd>
              </div>
            </dl>
          </div>
        </div>
