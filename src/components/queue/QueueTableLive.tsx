...
export function QueueTableLive({
  ...
  const [pagination, setPagination] = useState<PaginationData>(
    initialPagination || { page: initialPage, pageSize: 25, totalCount: initialJobs.length, totalPages: 1 }
  );
  ...
  useEffect(() => {
    ...
    eventSource.addEventListener("message", (event) => {
      try {
        ...
        if (pagination.page === 1 && data.pagination.totalCount > pagination.totalCount) {
          setHasNewJobs(true);
        }
