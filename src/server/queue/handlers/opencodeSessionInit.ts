...
export async function handleOpencodeSessionInit(
	ctx: QueueJobContext,
): Promise<void> {
	...
	if (!project) {
		logger.warn(
			...
			"Project not found for opencode.sessionInit",
		);
		return;
	}
	...
	if (project.status === "deleting") {
		logger.info(
			...
			"Skipping opencode.sessionInit for deleting project",
		);
		return;
	}
	...
	try {
		...
		if (!firstMessageId) {
			...
			const messagesResponse2 = await client.session.messages({
				sessionID: sessionId,
			});
			const messagesData2 = messagesResponse2.data as
				| Array<{ info?: { id?: string } }>
				| undefined;
		...
		logger.info(
			{ projectId: project.id, sessionId },
			"Initialized opencode session with agent",
		);
		...
		logger.debug(
			{ projectId: project.id, initMsgId: firstMessageId },
			"Stored init prompt message ID",
		);
