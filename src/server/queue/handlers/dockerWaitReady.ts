...
export async function handleDockerWaitReady(ctx: QueueJobContext): Promise<void> {
  ...
  try {
    ...
    await ctx.throwIfCancelRequested();
     ...
     logger.debug(
       { projectId: project.id, devPort: project.devPort, opencodePort: project.opencodePort },
     ...
     if (previewReady && opencodeReady) {
       ...
       logger.info({ projectId: project.id, elapsed, attempts: ctx.job.attempts }, "Services are ready");
