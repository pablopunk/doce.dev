 ...
 */
export async function handleOpencodeSendUserPrompt(ctx: QueueJobContext): Promise<void> {
  ...
  if (!project) {
    logger.warn({ projectId: payload.projectId }, "Project not found for opencode.sendUserPrompt");
    return;
  }

  if (project.status === "deleting") {
    logger.info({ projectId: project.id }, "Skipping opencode.sendUserPrompt for deleting project");
    return;
  }

  // If already sent, skip
  if (project.initialPromptSent) {
    logger.info({ projectId: project.id }, "User prompt already sent, skipping");
    return;
  }
  ...
  try {
    ...
    logger.debug({ projectId: project.id }, "User prompt sent, completion will be detected by event handler");
  } catch (error) {
    throw error;
  }
}
