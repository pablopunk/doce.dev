...
import { deleteMessage, deleteMessagesFromIndex, getConversation, getMessages } from "@/lib/db";

export const DELETE: APIRoute = async ({ params, request }) => {
  const { projectId, messageId } = params;

  if (!projectId || !messageId) {
    return Response.json({ error: "Project ID and Message ID are required" }, { status: 400 });
  }
  ...
  try {
    ...
    if (deleteFrom) {
      ...
      const deletedCount = deleteMessagesFromIndex(conversation.id, messageIndex);
    ...
    return Response.json({ error: error.message || "Failed to delete message" }, { status: 500 });
  }
};
